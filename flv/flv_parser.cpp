...
FLVParser::FLVParser() { memset(m_status, 0, sizeof(m_status)); }

FLVParser::~FLVParser() { m_file.close(); }
...
bool FLVParser::eof() const {
    return m_file.is_opened() ? m_file.eof() || m_file.cursor() >= m_file.size()
                              : true;
}

int FLVParser::read_header(FLVHeader &hdr, uint8_t *buf, uint32_t buf_size) {
    if (!buf) {
...
int FLVParser::read_tag(FLVTag *&tag, uint8_t *buf, uint32_t buf_size) {
    ...
    if (prev_tag_size != VALUI24(tag->hdr.datasize) + sizeof(FLVTagHeader)) {
        LOGE("Read \"Prev-Tag-Size\" failed, %u != %u + %u", prev_tag_size,
    ...
    return VALUI24(tag->hdr.datasize) + sizeof(FLVTagHeader) + sizeof(uint32_t);
}

int FLVParser::handle_script(AMFData &script, const byte strm[], uint32_t len) {
    const byte *p = strm;
...
int FLVParser::parse_avc(const byte *&p, uint32_t len, FLVVideoTagData &vdat) {
    const byte *savep = p;
    ...
    if (vdat.pkt.pkt_typ != NALU && composition_time != 0) {
    ...
    case NALU: {
        ...
        do {
            ...
            NaluItem *nitem = new NaluItem(nalu_len, new byte[nalu_len]);
...
int FLVParser::handle_video(FLVVideoTagData &vdat, const byte strm[],
                            uint32_t len) {
    const byte *p = strm;
...
int FLVParser::handle_audio(FLVAudioTagData &adat, const byte strm[],
                            uint32_t len) {
    const byte *p = strm;
    ...
    *(byte *)&adat = *p++;
    ...
    case 0: // AAC Sequence Header (2 bytes)
        ...
        generate_asc(adat.aac.asc, str_to_audioprof("LC"),
                     str_to_samplerate_idx("44100"), 2);
        if (len >= 4) {
...
int FLVParser::free_audio_tag_dat(FLVAudioTagData &adat) {
    if (adat.sound_fmt == 10 /* AAC */ && adat.aac.typ == 1 /* AAC-raw */) {
